...
int main(void) {
    ...
    // characteristics. This array is out of order by leg_count,
    // but we'll sort it in a second.
    struct animal a[4] = {
    ...
    /* As long as you give qsort() a function that can compare two items
     that you have in your array to be sorted, it can sort anything. And
     it does this without needing to have the types of the items hardcoded
     in there anywhere. qsort() just rearranges blocks of bytes based on
     the results of the compar() function you passed in.
    */
}
